...
const Newsletter = () => {
  ...
  const addToMailList = (data) => {
    ...
    fetch(`https://api.apilayer.com/email_verification/${data.email}`, {
      ...
      .then(res => res.json())
      .then(data => {
        console.log("data.is_deliverable", data.is_deliverable)
  ...
  return (
    <Container fluid className="newsletter-container d-flex justify-content-center py-5 px-0 mx-0">
      <Container fluid="xl">
        <Row className="d-flex flex-column flex-md-row align-items-center justify-content-center ">
          <Col>
            <h3 className="fw-bold pe-5">GET INSTANT DEALS AND EXCLUSIVE OFFERS!</h3>
          </Col>
          {isSucceed ?
            <SuccessDiv message="Thank you. You have successfully subscribed to the Jack's Garden newsletter." /> :
            <Col className='pt-3'>
              <Form className='input-group mb-2' onSubmit={handleSubmit(addToMailList)}>
                <FormControl name="email" type="text" placeholder="Enter your email to subscribe"
                ...
                <Button variant="success" type="submit"> {isFetching ?
                  <SpinnerDiv /> :
                  "Subscribe"}</Button>
              </Form>
              <div className="text-danger">{errors.email && errors.email.message}</div>
              <p>By providing your email you agree that your personal information will be handled in accordance with our privacy policy.</p>
            </Col>}
        </Row>
      </Container>
    </Container>
